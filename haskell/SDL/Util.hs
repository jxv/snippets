...
loadImage s ck = Sdl.load s >>= Sdl.displayFormat >>= setColorKey' ck
  where
    setColorKey' Nothing sur = return sur
    setColorKey' (Just(r,g,b)) sur  = do
      p <- (Sdl.mapRGB . Sdl.surfaceGetPixelFormat) sur r g b
      void $ Sdl.setColorKey sur [Sdl.SrcColorKey] p
      return sur


fromIntegral' :: (Integral a, Num b) => a -> a -> (b, b)
