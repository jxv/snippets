...
toValue str = case str of "+" => Operator Add
                          "-" => Operator Sub
                          "x" => Operator Mul
                          "/" => Operator Div
                          s   => Number (cast s)
...
eval ((Operator op)::vs) (n::m::ns) = let math = case op of Add => (+)
                                                            Sub => (-)
                                                            Mul => (*)
                                                            Div => (/)
...
main = do args <- System.getArgs
          case args of (_::input) => case eval (map toValue input) Nil of Right n => print n
                                                                          Left er => putStrLn er
                       _ => putStrLn "No input."
